...
// given key.
func (t *tOps) find(f *tFile, key []byte, ro *opt.ReadOptions, value []byte) (rkey []byte, err error) {
	ch, err := t.open(f)
	if err != nil {
		return nil, err
	}
	defer ch.Release()
	return ch.Value().(*table.Reader).Find(key, true, ro, value)
}
...
func (t *tOps) findKey(f *tFile, key []byte, ro *opt.ReadOptions, rkey []byte) (err error) {
	...
	defer ch.Release()
	return ch.Value().(*table.Reader).FindKey(key, true, ro, rkey)
}
